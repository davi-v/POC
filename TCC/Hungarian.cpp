#include "Pch.hpp"
// adapted from https://raw.githubusercontent.com/jamespayor/weighted-bipartite-perfect-matching/master/hungarian.cpp
...
static const auto oo = std::numeric_limits<double>::max();
...
struct LeftEdge {
    ...
    LeftEdge() : right(), cost() {}
    LeftEdge(int right, double cost) : right(right), cost(cost) {}
...
const std::vector<int> hungarianMinimumWeightPerfectMatching(const int n, const std::vector<WeightedBipartiteEdge> allEdges) {
    ...
    {
        std::vector<int> leftEdgeCounts(n), rightEdgeCounts(n);
        std::fill_n(leftEdgeCounts.data(), n, 0);
        std::fill_n(rightEdgeCounts.data(), n, 0);
    ...
    std::vector<double> leftPotential(n), rightPotential(n);
    ...
    fo(i, n) {
        ...
        auto smallestEdgeCost = edges[0].cost;
    ...
    std::fill_n(rightPotential.data(), n, oo);

    fo(edgeIndex, allEdges.size()) {
        const WeightedBipartiteEdge& edge = allEdges[edgeIndex];
        auto reducedCost = edge.cost - leftPotential[edge.left];
    ...
    std::vector<int> leftTightEdgesCount(n);
    std::fill_n(leftTightEdgesCount.data(), n, 0);
    ...
    fo(i, n) {
        ...
        fo(edgeIndex, edges.size()) {
            const LeftEdge& edge = edges[edgeIndex];
            auto reducedCost = edge.cost - leftPotential[i] - rightPotential[edge.right];
            if (reducedCost == 0) {
    ...
    std::vector<int> leftMatchedTo(n), rightMatchedTo(n);
    std::fill_n(leftMatchedTo.data(), n, UNMATCHED);
    std::fill_n(rightMatchedTo.data(), n, UNMATCHED);
    ...
    std::vector<double> rightMinimumSlack(n);
    std::vector<int> rightMinimumSlackLeftNode(n), rightMinimumSlackEdgeIndex(n);
    ...
    while (currentMatchingCardinality < n) {
        ...
        std::fill_n(rightMinimumSlack.data(), n, oo);
        std::fill_n(rightMinimumSlackLeftNode.data(), n, UNMATCHED);

        // Clear the queue.
        leftNodeQueue.clear();

        // Mark everything "unseen".
        std::fill(leftSeen.begin(), leftSeen.end(), false);
        std::fill(rightBacktrack.begin(), rightBacktrack.end(), UNMATCHED);
        ...
        while (endingRightNode == UNMATCHED) {
            ...
            while (endingRightNode == UNMATCHED && !leftNodeQueue.empty()) {
...
#ifdef _DEBUG
                    {
                        //auto t = edge.cost - leftPotential[i] - rightPotential[j];
                        //assert(edge.cost - leftPotential[i] - rightPotential[j] >= 0); // mudei para double em vez de int, então pode acontecer erro de precisão
                    }
#endif
                ...
                if (endingRightNode == UNMATCHED) {
                    const auto potential = leftPotential[i];
                    range(edgeIndex, leftTightEdgesCount[i], edges.size()) {
                        ...
                        if (rightMatchedTo[j] == UNMATCHED || !leftSeen[rightMatchedTo[j]]) {
                            ...
                            auto reducedCost = edge.cost - potential - rightPotential[j];
                            //assert(reducedCost >= 0); // double
